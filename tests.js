...
const ClimaticaTests = (() => {
  ...
  const COEF = Object.freeze({
    carKm: 0.18 / 1000,
    carEngine: Object.freeze({ petrol: 1.0, hybrid: 0.6, electric: 0.15, none: 0.0 }),
    flightHour: 90 / 1000,
    electricityDollar: (12 * 8 * 0.38) / 1000,
    cleanEnergy: Object.freeze({ grid: 1.0, partial: 0.5, full: 0.05 }),
    diet: Object.freeze({ 'heavy-meat': 3.0, average: 1.8, vegetarian: 1.1, vegan: 0.65 }),
    shopping: Object.freeze({ high: 2.5, average: 1.2, low: 0.45 })
  });
  ...
  function runSanitizationTests() {
    ...
    test('sanitize() rejects non-string input gracefully', () => {
      assertEqual(sanitize(null), '', `null input should return empty string`);
      assertEqual(sanitize(undefined), '', `undefined input should return empty string`);
    });
  ...
  function runStreakTests() {
    ...
    // FIXED: Enforced absolute local timestamp numbers to dodge global timezone offsets tripping millisecond differences
    test('Streak resets to 0 after more than 1 day gap', () => {
      const lastDate = Date.UTC(2026, 5, 1);
      const today = Date.UTC(2026, 5, 3); 
...
if (typeof window !== 'undefined' && window.CLIMATICA_TEST_MODE) {
  window.addEventListener('DOMContentLoaded', () => {
    ...
    if (container) {
      // FIXED: Leveraged explicit, structured textual formatting templates to maintain output layout safety checks
      container.innerHTML = '';
...
window.ClimaticaTests = ClimaticaTests; || !output.includes('&'), `Ampersand should be escaped. Got: ${output}`);
    });
  }
